...
(
	SELECT	* 
	FROM	dbo.SYSOBJECTS 
	WHERE	Id = OBJECT_ID(N'[dbo].[SpAccountPositionUpdate]')
	AND		OBJECTPROPERTY(Id, N'ISPROCEDURE') = 1
)
BEGIN
	DROP PROCEDURE [dbo].[SpAccountPositionUpdate]
END
GO
--==============================================================================================================================================
--	Name:		 				dbo.SpAccountPositionUpdate
...
EXEC SpAccountPositionUpdate @pUserId = '71722361-99FF-493F-AF02-2BD0ED7CE676', @pAccountPositions =
       '[
...
*/
--==============================================================================================================================================
CREATE PROCEDURE [dbo].[SpAccountPositionUpdate]
@pUserId UNIQUEIDENTIFIER,
@pAccountPositions VARCHAR(MAX)
AS
SET NOCOUNT ON
...
DECLARE @AccountPositionTmpTable TABLE(
	AccountId INT,
	Position INT
);
...
BEGIN TRY
	...
	INSERT INTO @AccountPositionTmpTable (AccountId, Position)
	Select
	...
	From parseJSON(@pAccountPositions)
	where ValueType = 'int'
	group by parent_ID

	UPDATE dbo.Account SET Position = accptm.Position
	FROM @AccountPositionTmpTable accptm
	JOIN dbo.Account acc ON acc.AccountId = accptm.AccountId
	WHERE acc.UserId = @pUserId;

	SELECT acc.AccountId, CAST(1 AS BIT) AS Affected
	FROM @AccountPositionTmpTable accptm
	JOIN dbo.Account acc ON acc.AccountId = accptm.AccountId
	WHERE acc.UserId = @pUserId;
