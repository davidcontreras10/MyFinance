...
(
	SELECT	* 
	FROM	dbo.SYSOBJECTS 
	WHERE	Id = OBJECT_ID(N'[dbo].[SpAccountOrderList]')
	AND		OBJECTPROPERTY(Id, N'ISPROCEDURE') = 1
)
BEGIN
	DROP PROCEDURE [dbo].[SpAccountOrderList]
END
GO
--==============================================================================================================================================
--	Name:		 				dbo.SpAccountOrderList
...
INSERT INTO @p1 VALUES (4006),(3004),(5),(4),(2003),(3004)

EXEC SpAccountOrderList @pUserId = '71722361-99FF-493F-AF02-2BD0ED7CE676', @pAccountIds = @p1;

END

*/
--==============================================================================================================================================
CREATE PROCEDURE [dbo].[SpAccountOrderList]
...
DECLARE @RtnAccountOrderedTable TABLE(
	AccountId INT,
	AccountName VARCHAR(500),
	GlobalOrder INT IDENTITY(1, 1)
);
...
BEGIN TRY
	...
	INSERT INTO @RtnAccountOrderedTable (AccountId, AccountName)
	SELECT acc.AccountId, acc.Name
	FROM @pAccountIds tmpids
	JOIN dbo.Account acc ON acc.AccountId = tmpids.Value
	JOIN dbo.AccountGroup accg ON accg.AccountGroupId = acc.AccountGroupId
	ORDER BY accg.AccountGroupPosition, acc.Position

	SELECT * FROM @RtnAccountOrderedTable;
