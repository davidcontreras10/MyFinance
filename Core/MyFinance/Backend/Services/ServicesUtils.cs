...
using Ut = MyFinance.Utilities.SystemDataUtilities;
...
namespace MyFinance.Backend.Services
{
    internal static class ServicesUtils
    {
	    ...
	    public static Spend CreateSpend(DataRow dataRow)
	    {
	        return dataRow == null
	            ...
	            : new Spend
	            {
	                ...
	                AccountIdsArray = CreateAccountIdsArray(dataRow, DatabaseConstants.COL_ACCOUNT_IDS),
	                AccountIds =
        ...
        private static int[] CreateAccountIdsArray(DataRow dataRow, string columnName)
        {
            if (dataRow == null || string.IsNullOrWhiteSpace(columnName) || !dataRow.Table.Columns.Contains(columnName))
                return new int[] { };
            string value = Ut.GetString(dataRow, columnName);
